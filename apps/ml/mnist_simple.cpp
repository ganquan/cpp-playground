...
int main() {
    ...
    const char *dataPathCStr = getenv("MLPACK_DATA_PATH");
    std::string train_data_path;
    std::string test_data_path;
    if (nullptr != dataPathCStr) {
        std::string dataPath;
        dataPath = std::string(dataPathCStr);
        train_data_path = fmt::format("{}/mnist_train.csv", dataPath);
        test_data_path = fmt::format("{}/mnist_test.csv", dataPath);
    } else {
        spdlog::info("check environment variable MLPACK_DATA_PATH");
        return 0;
    }
    data::Load(train_data_path, dataset, true);
    ...
    data::Load(test_data_path, dataset, true);
